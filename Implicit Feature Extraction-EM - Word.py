...
DataFile='iPod.final'
Results_Directory='./Results/iPod/EM_word/'
...
for GlobalIteration in range(0,1):
  ...
  CorrelatedWords={}
  for aspect in ExplicitFeatures:
    CorrelatedWords[aspect]={}
  #for MyK in range(0,len(ExplicitFeatures)+1):
  MyK=0
  for ColorProbability in np.arange(0.2, 0.24, 0.05):
    Theta=0.0
    TP=0
    FP=0
    FN=0
    ...
    for reviewNum in range(0,len(Reviews)):
      for lineNum in range(0,len(Reviews[reviewNum])):
        if '[u]' in RawReviews[reviewNum][lineNum]:
          ...
          for word in Reviews[reviewNum][lineNum]:
            for aspect in sorted(HP[reviewNum][lineNum][word], key=HP[reviewNum][lineNum][word].get, reverse=True):
              if HP[reviewNum][lineNum][word][aspect]*(1-HPB[reviewNum][lineNum][word])>=ColorProbability and aspect not in ActualExplicitFeatureSet and aspect not in Reviews[reviewNum][lineNum] and ExplicitFeatures[aspect]>=FrequentThreshold and aspect not in InferredImplicitFeatureSet:
                  #print(word, aspect, HP[reviewNum][lineNum][word][aspect],HPB[reviewNum][lineNum][word])
                  InferredImplicitFeatureSet.add(aspect)
                  if word in CorrelatedWords[aspect]:
                    CorrelatedWords[aspect][word]+=1
                  else:
                    CorrelatedWords[aspect][word]=1
...
with open(Results_Directory+'CorrelatedWords.txt','w') as outputfile:
  for aspect in CorrelatedWords:
    outputfile.write(aspect+'\n--------------------------\n')
    for word in sorted(CorrelatedWords[aspect], key=CorrelatedWords[aspect].get, reverse=True):
      outputfile.write(word+','+str(CorrelatedWords[aspect][word])+'\n')
    outputfile.write('\n\n\n')
